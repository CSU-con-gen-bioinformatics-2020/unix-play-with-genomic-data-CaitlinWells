...
```
to check that you're not deleting more files or directories than you intend!
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
5 files or directories called "bad" "directory" "name" "with" and "spaces," which don't exist
```
...
```
They are the same for the pairs of reads (except for the /1 and /2 designations at the ends)
```
